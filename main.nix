...
let
  unknown = x: set:
    if (builtins.isAttrs set) && (builtins.hasAttr x set) then
      builtins.getAttr x set
    else
      "unknown";
  getDescription = pkg:
    if builtins.hasAttr "meta" pkg then
      if builtins.hasAttr "description" pkg.meta then
  ...
  getLongDescription = pkg:
    if builtins.hasAttr "meta" pkg then
      if builtins.hasAttr "longDescription" pkg.meta then
